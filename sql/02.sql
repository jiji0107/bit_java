...
select ename,sal,sal*12 as 년봉,comm from emp; -- 오라클에선as 생략 가능


select ename,sal,sal*12 as "년    봉",comm from emp;  --컬럼네임 얼라이언스 시킬때만 더블쓰기
...
select empno,ename,sal,comm,sal+comm 월급여 from emp; --널이 없는것만 나온다

select empno,ename,sal,comm,nvl(comm,0) from emp; --comm이 널이면 0으로 바꿔줘
...
select empno,ename,nvl(mgr,0),deptno from emp;
select empno,ename,nvl(mgr,'CEO'),deptno from emp; -- mgr과 'CEO'의 데이터타입 불일치
select empno,ename,nvl(to_char(mgr),'CEO'),deptno from emp;  --mgr을 문자로 형변환
...
select 10*24*24 from dual; -- dual 가상의 테이블
...
select * from emp where job='MANAGER'; -- 대소문자 구분 해야한다.

## 대소문자 구분하지 않고 검색 가능하도록 합니다.
select * from emp where upper(job) = upper('MANAGER');

select * from emp where hiredate = sysdate;
select * from emp where hiredate = '91/09/28';
...
select * from dept where (deptno,loc) IN((10, 'DALLAS'),(30,'CHICAGO')); --(10,DALLAS) 사이는 and 조건 두개의 조건 사이 , 은 or처리
...
select * from emp where ename like 'A%' or ename like 'B%'or ename like 'C%'or ename like 'D%';

##정규식 사용
select * from emp where regexp_like (ename, 'A|B|C|D'); --ABCD를포함하는걸로 결과가 나온다
select * from emp where regexp_like (ename, '[A-D]');
select * from emp where regexp_like (ename, '^[A-D]'); -- ^시작을 의미
select * from emp where regexp_like (ename, '[A-D]$'); -- $ 끝글자가 ABCD중하나야한다.
...
select ename, sal, comm,sal+ nvl(comm,0) total ,dept
from emp
order by deptno;
...
select ename, sal, comm,sal+ nvl(comm,0) total ,dept
from emp
order by deptno desc,total; -- deptno는 오름차순 토탈은 내림차순으로
...
select *  from dept;
select dname,loc from dept;
select dname,lower(dname),loc,lower(loc) from dept; -- return 타입이 소문자 ()이걸 내장함수라고한다. 

select round (45.5678) , trunc(45.5678) from dual;
select round (45.5678,2) , trunc(45.5678,2) from dual;

select sysdate , sysdate+1 from dual; -- 날짜가 하루 증가
...
## nvl(a,b) 함수
select ename, mgr ,nvl(comm,'_') from emp; -- comm 숫자 _ 문자타입이라 타입일치X
...
select ename, mvl2(mgr,'O','X') ,nvl(comm,0)from emp; -- mgr이 있으면 O 없으면 X

## decode 함수
select ename , sal , deptno , decode(deptno, 10, sal*1,
											 20, sal*2,
											 30, sal*3,
											 sal) 계산 from emp; -- 얼라이언트 (계산 from emp;)부분이 계산할때 사용한다 디코드함수
